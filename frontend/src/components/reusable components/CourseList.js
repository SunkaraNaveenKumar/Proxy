...
const CourseList = ({ courses, type, userId }) => {
  ...
  const { role } = useAuth();
  const [enrollUser, { data: enrolledUserData, isLoading: isEnrolling }] =
    useEnrollUserMutation();
  const [unEnrollUser, { data: unEnrollUserData, isLoading: isUnEnrolling }] =
    useUnEnrollUserMutation();
  ///// useEffect
                ...
                {type === "enrolledCourses" ? (
                  role === "admin" && (
                    <>
                      <button
                        onClick={() => {
                          handleUserEnroll(_id);
                        }}
                        disabled={enrolled}
                        className={clsx(
                          "shawdow-lg self-stretch w-20 rounded-lg bg-blue-300 border border-black border-solid",
                          { "opacity-30": enrolled }
                        )}
                      >
                        Enroll
